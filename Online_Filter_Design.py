...
st.set_page_config(page_title="Motion Tools", page_icon="⚙️", layout="wide")
...
with tab_profile:
    ...
    def generate_profile(start, finish, vel, acc, jerk, mass_kg, kt, unit_scale, friction_n,
        ...
        if motor_type == "Linear (V/(m/s))":
            speed_si = [s * unit_scale for s in speed]
            bemf     = [ke * abs(s) for s in speed_si]
        else:
            bemf = [ke * abs(s) for s in speed]
        ...
        voltage = [b + abs(i) * resistance for b, i in zip(bemf, current)] if bus_voltage is not None else None
        ...
        def make_fig(x, y, title, ytitle, hover_y, extra_traces=None):
            ...
            fig.update_layout(title=title, xaxis_title="Time [sec]", yaxis_title=ytitle,
        ...
        if voltage is not None:
            ...
            fig_volt = make_fig(time, voltage, "Required Motor Voltage (BEMF + I·R)",
                                "Voltage (V)", "V_motor", extra_traces=[bus_line])
            fig_volt.update_layout(showlegend=True)

        results = dict(
            t_total=t_total, acc_rms=acc_rms,
            current_rms_profile=current_rms_profile,
            current_rms_total=current_rms_total,
            current_peak=current_peak,
            bemf_peak=bemf_peak, ir_peak=ir_peak,
            v_required=v_required, bus_ok=bus_ok, bus_margin=bus_margin,
        )
    ...
    with ctrl_col:
        ...
        position_unit = st.selectbox(
            "What do User Units (UU) represent?", ["Millimeters", "Meters"], index=0,
        ...
        friction_n = st.number_input("Friction force (N)",                       value=0.0,  min_value=0.0,  step=0.1,  format="%.3f",
        ...
        duty_cycle_pct = st.number_input(
            ...
            help="Fraction of time the move is active. I_rms_total = I_rms_profile × √(duty).",
        )

        st.markdown("---")
        st.caption("Bus voltage check (optional)")

        motor_type = st.selectbox("Motor type", ["Linear (V/(m/s))", "Rotary (V/RPM)"],
        ...
        if run_profile1 or run_profile2:
            try:
                ...
                c3.metric(f"RMS Current (duty={duty_cycle_pct:.0f}%)",
                          ...
                          help="I_rms_profile × √(duty) — thermal RMS seen by motor/drive.")
                ...
                if bus_voltage is not None:
                    ...
                    if res['bus_ok']:
                        v4.metric("Bus margin", margin_str, delta=margin_str, delta_color="normal")
                        st.success(f"✅ Bus sufficient — {margin:.2f} V headroom above peak demand.")
                    else:
                        v4.metric("Bus deficit", margin_str, delta=margin_str, delta_color="inverse")
...
with tab_filter:
    ...
    def calculate_biquad_params(filter_type, f_hz, width_hz, atten_db, lpf_nd=0.707, lpf_dd=0.707):
        ...
        if filter_type == 'notch':
            ...
            nd = lpf_nd   # user-defined
            dd = lpf_dd   # user-defined
    ...
    with ctrl_col2:
        ...
        if filter_type == "manual":
            ...
            """)
            man_nf = st.number_input("SLVB0NF — Numerator Frequency, ωN (Hz)",
                min_value=0.1, max_value=4000.0, value=100.0, step=1.0, format="%.2f",
                help="Valid range: 0.1 – 4000 Hz")
            man_df = st.number_input("SLVB0DF — Denominator Frequency, ωD (Hz)",
                min_value=0.1, max_value=4000.0, value=100.0, step=1.0, format="%.2f",
                help="Valid range: 0.1 – 4000 Hz")
            man_nd = st.number_input("SLVB0ND — Numerator Damping, ζN",
                min_value=0.01, max_value=1.0, value=0.1, step=0.01, format="%.4f",
                help="Valid range: 0.01 – 1.0")
            man_dd = st.number_input("SLVB0DD — Denominator Damping, ζD",
                min_value=0.01, max_value=1.0, value=0.5, step=0.01, format="%.4f",
                help="Valid range: 0.01 – 1.0")
            lpf_nd = lpf_dd = None  # not used in manual mode

        else:
            freq = st.number_input("Center / cutoff frequency (Hz)",
                min_value=0.1, max_value=4000.0, value=100.0, step=1.0, format="%.1f")

            if filter_type != "lpf":
                width = st.number_input("Filter width (Hz)",
                    min_value=0.1, max_value=4000.0, value=20.0, step=1.0, format="%.1f")
            else:
                width = 0.0
                st.caption("Width is not used for LPF.")

            atten = st.number_input("Attenuation (dB)", min_value=0.01, value=5.0, step=0.1, format="%.2f")
        ...
        if run_filter:
            try:
                if filter_type == "manual":
                    ...
                    nf, df, nd, dd = calculate_biquad_params(
                        filter_type, freq, width, atten,
                        lpf_nd=lpf_nd or 0.707,
                        lpf_dd=lpf_dd or 0.707,
                    )
